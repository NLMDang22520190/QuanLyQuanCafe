...
export const MenuPage = () => {
  ...
  const changeProductState = async (id) => {
    ...
    try {
      const response = await api.put(
        `api/menu-items/${id}/change-availability`
      );
      const data = response.data;
  ...
  const columnData = [
    ...
    },
    {
      ...
      dataIndex: "price",
      key: "price",
      render: (text) => `${parseFloat(text).toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}`,
    ...
    },
    {
      ...
      render: (text, record) => (
        <Switch
          loading={availabilityLoading == record.itemId}
          defaultChecked={text === "Available"}
          onChange={() => changeProductState(record.itemId)}
        />
      ),
    },
    {
        ...
        <Button
          onClick={() => {
            setIsProductDetailModalVisible(true),
              setSelectedProductId(record.itemId);
          }}
          type="text"
        >
          <svg
  ...
  useEffect(() => {
    if (menuItems.length > 0) {
      setCurrentMenuItems(
        menuItems.filter((item) => item.typeOfFoodId === currentTypeOfFoodId)
      );
    }
  ...
  const onSearchCategories = (value) => {
    const filteredTypeOfFoods = typeOfFoods.filter((type) =>
      type.typeOfFoodName.toLowerCase().includes(value.toLowerCase())
    );
        ...
        <div className="flex justify-between">
          ...
          <div className="flex gap-x-4">
            ...
            <Button
              ...
              onClick={setIsAddCategoryModalVisible}
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                ...
                <li key={type.typeOfFoodName} role="presentation">
                  <button
                    ...
                    className={`inline-block p-4 border-b-2 rounded-t-lg ${
                      currentTypeOfFoodId === type.typeOfFoodId
