...
const Schedule = () => {
  ...
  const fetchStaffsNotInShift = async () => {
    try {
      if (selectedShiftInTable === null) return;
      const response = await instance.get(
        `api/staff/staff-not-in-shift/${
          selectedShiftInTable.shiftId
        }?month=${selectedMonth.format("M")}&year=${selectedMonth.format(
          "YYYY"
        )}`
      );
  ...
  const fetchShift = async (pageIndexMShift, pageSize) => {
    try {
      const response = await instance.get(
        `/api/shifts?pageIndex=${pageIndexMShift}&pageSize=${pageSize}`
      );
      console.log(response.data);
  ...
  const fetchShiftSchedule = async () => {
    const response = await instance.get(
      `/api/shifts?pageIndex=${1}&pageSize=${30}`
    );
  ...
  const handleEditShift = async () => {
    try {
      ...
      const response = await fetch(
        `https://localhost:7087/api/shifts/${selectedShift.shiftId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error: ${response.statusText}. ${errorData.error || "Unknown error"}`
        );
      }
    ...
    try {
      ...
      const attendanceData = response.data.data.map((record) => ({
        ...
        checkIn: record.checkIn
          ? moment(record.checkIn).format("HH:mm:ss")
          : "Not checkin",
        checkOut: record.checkOut
          ? moment(record.checkOut).format("HH:mm:ss")
          : "Not checkout",
      }));
  ...
  const mainShiftColumns = [
    ...
    { title: "End Time", dataIndex: "endTime", key: "endTime" },
    {
      ...
      render: (_, record) => (
        <>
          <Button type="default" onClick={() => handleEditClick(record)}>
            Edit
          </Button>
          <Button
            type="default"
        ...
        if (editingKey === record.staffId) {
          return (
            <div>
              ...
              />
              <Button
                onClick={() => handleSaveEndDate(record.staffId)}
                type="default"
                style={{ marginLeft: 8 }}
              >
                Save
  ...
  const handleUpdateEndDate = (key) => {
    setEditingKey(key);
    const currentRecord = staffAssignedToShift.find(
      (record) => record.staffId === key
    );
        ...
        >
          <div>
            <div className="flex justify-between items-center">
              <div className="mt-4 flex  items-center">
                ...
                <Select
                  value={
                    assignStartDate
                      ? assignStartDate.format("YYYY-MM-DD")
                      : null
                  }
                ...
                <DatePicker
                  ...
                  onChange={(value) => {
                    if (
                      assignStartDate &&
                      dayjs(value).isSameOrBefore(assignStartDate, "day")
                    ) {
                  ...
                  disabledDate={(currentDate) => {
                    return (
                      assignStartDate &&
                      currentDate.isSameOrBefore(assignStartDate, "day")
                    );
                  }}
                />
            ...
            <Table
              title={() => (
                <>
                  <span className="custom-table-title mr-8">
                    Monthly Assignment
                  </span>
                  <DatePicker
          ...
          {selectedShift && (
            <div>
              ...
              <p>
                <strong>Time:</strong>{" "}
                {`${
                  selectedShift?.StartTime?.toLocaleTimeString() || "00:00:00"
                } - ${
                  selectedShift?.EndTime?.toLocaleTimeString() || "00:00:00"
                }`}
              </p>
