...
export const OrderAndBilling = () => {
  ...
  const updateOrderStatus = async (status) => {
    ...
    const response = await api
      .put(`api/Order/update-order-state/${orderId}`, state)
      .then((response) => {
        fetchOrder();
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (orderTime) => <p>{new Date(orderTime).toLocaleString()}</p>,
    },
    {
      title: "Order Detail",
    ...
    const filteredOrders = orders.filter(
      (order) =>
        ...
        order.orderDetails.some((detail) =>
          detail.item.itemName.toLowerCase().includes(value.toLowerCase())
        )
    );
  ...
  return (
    <>
      <div className="flex flex-col gap-y-4 overflow-hidden h-full">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-x-2">
            ...
            <Button
              onClick={() => navigate("/orderAndBilling/create")}
              type="primary"
      ...
      >
        {selectedOrder && (
          <div className="flex flex-col gap-y-2">
            ...
            <p>
              <strong>Date & Time:</strong> {selectedOrder.orderTime}
            </p>
      ...
      <Modal
        ...
        onOk={() => setOrderExportVisible(false)}
        onCancel={() => setOrderExportVisible(false)}
      >
