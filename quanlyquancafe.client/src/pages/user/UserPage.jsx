...
const UserPage = () => {
  ...
  const [currentTab, setCurrentTab] = useState(userRole === "Admin" ? 0 : 2);
  ...
  const fetchFStaffData = async () => {
    try {
      const response = await instance.get(
        `/api/staff/former-staffs?pageIndex=${pageIndexFStaff}&pageSize=${10}`
      );
  ...
  const fetchStaffData = async () => {
    try {
      const response = await instance.get(
        `/api/staff/current-staffs?pageIndex=${pageIndexStaff}&pageSize=${10}`
      );
  ...
  const fetchUsersData = async () => {
    try {
      const response = await instance.get(
        `/api/account/users?pageIndex=${pageIndexUser}&pageSize=${10}`
      );
      ...
      render: (text, record) => {
        ...
        return (
          <Select
            ...
            onChange={(value) =>
              handleStatusChange(value === "Active", record.id)
            }
            style={{ width: 120 }}
          >
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        <Button
          type="primary"
          ghost
          onClick={() => handleViewOrderList(record.id)}
        >
          View Orders
        </Button>
      )},
  ...
  const handleDisableStaff = async (userId) => {
    try {
      const response = await instance.post(
        `/api/staff/disable?userId=${userId}`
      );
  ...
  const handleCancel = () => {
    const updatedData = usersData.map((user) =>
      user.id === selectedUserId ? { ...user, role: previousRole } : user
    );
    setUsersData(updatedData);
      ...
      <div className="flex flex-col gap-y-4 overflow-hidden h-full">
            ...
            >
              {categories.map((tab, index) => (
                <li key={tab.name} role="presentation">
                  <button
                    onClick={() =>
                      handleTabChange(index + (userRole === "Admin" ? 0 : 2))
                    }
                    className={`inline-block p-4 border-b-2 rounded-t-lg ${
                      currentTab === index + (userRole === "Admin" ? 0 : 2)
        ...
        {currentTab === 1 && (
          <>
            <Table
              title={() => (
                <span className="custom-table-title">Current Staffs</span>
              )}
            ...
            <Table
              title={() => (
                <span className="custom-table-title">Former Staffs</span>
              )}
